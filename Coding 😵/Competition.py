...
class DroneGUI:
    def __init__(self, root):
        ...
        # QR Message pane (persistent)
        self.qr_message_pane = tk.Label(root, text="No QR detected", bg="lightblue", relief="sunken", anchor="w")
        self.qr_message_pane.grid(row=7, column=0, columnspan=2, sticky="we")

        # Status Message pane
        self.message_pane = tk.Label(root, text="Ready", bg="white", relief="sunken", anchor="w")
        self.message_pane.grid(row=8, column=0, columnspan=2, sticky="we")
    ...
    def execute_mission(self):
        try:
            ...
            Home.alt = float(self.alt_entry.get())

            # First takeoff
            self.arm_and_takeoff(float(self.alt_entry.get()))
            self.vehicle.airspeed = float(self.speed_entry.get())
            
            # Go to target location
            target_lat = float(self.lat_entry.get())
            target_lon = float(self.lon_entry.get())
            ...
            self.set_servo(self.vehicle, 6, 2000)
            ...
            self.set_servo(self.vehicle, 6, 1100)
