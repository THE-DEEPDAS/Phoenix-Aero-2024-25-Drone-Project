...
class DroneControlGUI:
    ...
    def connect_drone(self):
        try:
            # Use argparse to handle connection string
            import argparse
            parser = argparse.ArgumentParser()
            connection_string = self.ip_port_entry.get()
            parser.add_argument('--connect', default=connection_string)
            args = parser.parse_args(['--connect', connection_string])
            
            self.log_message(f"Connecting to vehicle on: {args.connect}")
            ...
            # For simulation, keep the mock vehicle if connection fails
            ...
            class Fv:
                def __init__(self):
                    ...
                    self.mode = "GUIDED"
                    self.location = type('',(),{})()
                    self.location.global_relative_frame = type('',(),{})()
                    self.location.global_relative_frame.alt = 0.0
                    self.airspeed = 0.0
                def simple_takeoff(self, alt):
                    self.location.global_relative_frame.alt = alt
                def simple_goto(self, target_location, groundspeed=0):
                    pass
            
            self.vehicle = Fv()
            self.is_connected = True
            self.msg_queue.put(("connection", True))
            self.log_message("Simulation mode activated")
            self.update_telemetry()
    ...
    def adjust_and_descend_to_qr(self, target_altitude=1.0):
        ...
        if not cap.isOpened():
            self.log_message("Error: could not open camera feed")
            return False
